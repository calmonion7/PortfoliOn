...
내비게이션은 5개 탭(+ admin 전용 1개)으로 구성된다. 홈(`/`)은 리서치다. 상단 헤더(데스크톱 nav·모바일 헤더)에는 **전역 종목 검색**이 있어 어느 화면에서든 종목을 찾을 수 있다 — 추적(보유/관심) 종목이면 그 리포트 상세로 점프하고, 미추적 종목이면 관심종목 추가 모달이 프리필로 열린다(발굴).

...
| 분석 탭 | 섹터 모멘텀(US=11개 섹터 ETF / KR=KRX 업종 모멘텀, KR/US 토글), 매크로 상관(TLT/UUP/USO/VIX), 보유 종목 상관관계 히트맵, 노출(통화·섹터·단일종목 비중 + 집중도·임계 초과 경고 + 포트폴리오 베타 + **기술 노출 상한** — 주요기술 리포트 `players[].ticker`와 내 보유를 교차한 기술별 비중 합, ADR-0043. 그 업체의 전사 비중이라 실제 노출의 *상한*이고 한 종목이 여러 기술에 들어가 **기술 간 합은 100%를 넘는다** — 그래서 임계 경고를 붙이지 않는다. 관심종목은 수량이 없어 막대에서 제외하고 개수만 부기) |

...
| 애널리스트 리포트 | 선별 종목 대상 발행물 누적형 심층 문서(`/analyst-report/:ticker/:date`, ADR-0027) — 증권사 리포트식 단일 스크롤(투자의견·적정주가 밴드·한줄 논지·투자 포인트·밸류에이션(산정방식+PER 밴드 차트+피어 멀티플)·실적 추정 테이블·리스크). 판단·서사는 Cowork가 온디맨드 발행(API), 숫자 블록은 서버가 발행 시점 스냅샷에서 자동 박제. 진입점: 리포트 탭 목록의 발행물 섹션 + 발행물 있는 종목의 리포트 상세 헤더 링크(없으면 숨김) |

...
> 백엔드에는 **LLM/Anthropic 호출이 없다**. AI 분석 텍스트는 claude.ai **루틴**(일일 리포트 배치 완료 시 백엔드가 fire로 깨우는 클라우드 에이전트, ADR-0028)이 enrich·애널리스트 리포트 발행·선도기술 리포트 발행 API(`CLAUDE_COWORK_API.md`)로 작성하며, 백엔드 리포트 생성은 시장 데이터 스냅샷만 만든다. 수동 트리거는 `POST /api/admin/cowork/fire`.
...
| 선도기술 | 선도기술 리포트 허브(`/tech-reports`) — **종목이 아니라 기술 단위**의 발행물(ADR-0033). 대상 4종(재사용 로켓·전고체 배터리·SMR·로봇, 백엔드 상수가 정본)의 목록(기술당 최신 1건 — 표시명·한줄 제목·발행일·기술난이도·업체 수·시장 규모 요약) → 상세(`/tech-report/:slug`)는 **지표·표 먼저, 산문은 하단** 구성: 기술명 헤더 + 한줄 제목 리드 → KPI 스트립(기술난이도·선두 업체·시장 규모·CAGR·주요 업체 수·양산상용 업체 수, 결측은 `—`) → 전역 목차(렌더된 섹션에서 파생한 앵커 칩, sticky 아님) → 핵심 포인트 카드(결론 3~4장 — 지표 칩 최대 4개 + 1~2문장) → 진척 타임라인(연도축 SVG, 상태 마커 완료·진행 중·예정) → 주요업체 표(업체·기술 성숙 단계 1~5·선두 대비 격차 년수·시장점유율 — 전 행 결측 열은 생략되고 국가·정부주도 배지·티커·보유/관심 배지는 업체 셀 안 메타줄, 비상장·국영 업체도 포함, 업체별 코멘트는 상시 노출·모바일 가로 스크롤 0) → 기술수준 비교 밴드 → 계열 비교(축마다 2열 표 — 계열·대표 사례 | 이점·대가, 계보 분류와 병존) → 계보 분류(계열별 업체 그룹 칩) → 점유율 차트 → 기술 난제 → 확인할 지표(번호 체크리스트 — 판정문 + 「신호 아님」 경고를 본문과 색으로 분리) → 시장 규모 성장 곡선(실측 실선/예상 점선) + 기관별 시장 추정치(조사기관별 가로 막대, 있을 때만 — 최대·최소 배수·성장 곡선 채택 기관 마커) → 연관 기술 관계도(전제→대상→파생 3열 + 보완·경합 칩) → 상세 설명(산문 전문을 대괄호 헤딩 기준 소제목으로 승격 — 접기 없이 상시 노출, 문단 단위 분리) → 출처. 데이터가 없는 섹션은 생략된다. 애널리스트 리포트와 달리 서버가 발행 시점에 자동 첨부하는 숫자가 전혀 없어(전방 시장 데이터 소스 부재) 판단·수치 전부를 루틴이 조사해 발행한다 |

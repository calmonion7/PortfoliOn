...
PC는 **좌측 사이드바** 5섹션(리서치·포트폴리오·시장·일정·인컴·구루) + 하단 설정·행동(admin)으로, 모바일은 하단 **MobileNav**로 내비게이션한다(상단 가로 nav 폐지 — 프로 금융 터미널 아이덴티티·다크 기본, ADR-0025). 사이드바 각 항목은 **개별 URL 라우트**라 딥링크·새로고침 복원이 된다(구 URL은 리다이렉트). 홈(`/`)은 리서치(`/reports`)로 리다이렉트된다. 상단 헤더에는 **전역 종목 검색**이 있어 어느 화면에서든 종목을 찾을 수 있다 — 추적(보유/관심) 종목이면 그 리포트 상세로 점프하고, 미추적 종목이면 관심종목 추가 모달이 프리필로 열린다(발굴).

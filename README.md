...
| 주요기술 | 주요기술 리포트 허브(`/tech-reports`) — **종목이 아니라 기술 단위**의 발행물(ADR-0033, 저장모델 개정 ADR-0038, 대상 개정 ADR-0039). 대상은 백엔드 상수 `TECH_TOPICS`가 정본이고(ADR-0039 1차 개정 → ADR-0044 2차 개정으로 15종), 편입 판정은 「지금 투자 지형에서 중요한가」다 — 넓은 이름은 한 문서가 감당할 범위로 좁혀 등재한다(예: 양자는 컴퓨팅만). 그 목록(기술당 1건 — 이력 없음, 재발행은 그 행을 덮어쓰는 갱신, 표시명·한줄 제목·마지막 갱신일·기술난이도·업체 수·시장 규모 요약). **그 발행물 그리드 아래에는 「발행 대기」 구역**이 있어 **등록됐지만 아직 발행되지 않은 기술을 이름 칩으로** 보인다 — 목록 응답이 등록 정본 `topics[]`(15종, 발행 여부 무관)를 함께 내리고 화면이 발행물 `reports[]`와의 차집합을 그린다. 등록 대상과 발행물은 서로 다른 집합인데 화면이 발행물만 그려서 미발행 종이 **존재하지 않는 것처럼** 보이던 것을 없앤 것이다(칩을 누르면 그 기술 상세의 「아직 발행된 리포트가 없습니다」 안내로 가고, 발행이 진행돼 대기가 0이 되면 구역이 스스로 사라진다). → 상세(`/tech-report/:slug`)는 **지표·표 먼저, 산문은 하단** 구성: 기술명 헤더 + 한줄 제목 리드 → KPI 스트립(기술난이도·선두 업체·시장 규모·CAGR·주요 업체 수·양산상용 업체 수, 결측은 `—`) → 전역 목차(렌더된 섹션에서 파생한 앵커 칩, sticky 아님) → 핵심 포인트 카드(결론 3~4장 — 지표 칩 최대 4개 + 1~2문장) → 진척 타임라인(연도 그룹 세로 목록, 상태 마커 완료·진행 중·예정) → 주요업체 표(업체·기술 성숙 단계 1~5·선두 대비 격차 년수·시장점유율 — 전 행 결측 열은 생략되고 국가·정부주도 배지·티커·보유/관심 배지는 업체 셀 안 메타줄, 비상장·국영 업체도 포함, 업체별 코멘트는 상시 노출·모바일 가로 스크롤 0. 기술 성숙 단계는 채워진 칸 수(최대 5)로 그려지는 밴드로 표시되고 표 위에 단계 범례가 1줄 있다. 업체 분류 축이 채워진 판은 표가 축별 소제목 행으로 묶이고 그 소제목에 분야 선두가 병기되며, 없으면 평면 표) → 계열 비교(축마다 2열 표 — 계열·대표 사례 | 이점·대가, 업체 분류 축과 병존 — 계열의 성질 vs 업체의 소속) → 점유율 차트(업체 분류 축이 있으면 축별 막대 그룹, Σ 100% 초과 경고도 그룹 단위) → 기술 난제 → 확인할 지표(번호 체크리스트 — 판정문 + 「신호 아님」 경고를 본문과 색으로 분리) → 시장 규모 성장 곡선(실측 실선/예상 점선) + 기관별 시장 추정치(조사기관별 가로 막대, 있을 때만 — 최대·최소 배수·성장 곡선 채택 기관 마커) → 연관 기술 관계도(전제·선행 ↓ 대상 ↓ 파생·응용 세로 흐름 — 대상 강조, 방향 없는 보완·경합은 구분선 아래 칩. 자체 가로 스크롤러·말줄임 없이 어떤 폭에서도 리플로우) → 상세 설명(산문 전문을 대괄호 헤딩 기준 소제목으로 승격 — 접기 없이 상시 노출, 문단 단위 분리) → 출처. 데이터가 없는 섹션은 생략된다(별도 「계보 분류」 칩 섹션은 없음 — 표·점유율 그룹핑이 그 역할을 대신한다). 애널리스트 리포트와 달리 서버가 발행 시점에 자동 첨부하는 숫자가 전혀 없어(전방 시장 데이터 소스 부재) 판단·수치 전부를 루틴이 조사해 발행한다. 같은 기술의 **두 번째 시선**으로 「기술 해부」(`/tech-anatomy/:slug`, ADR-0042)가 있다 — 「이 기술 하나를 완성하려면 무엇이 얼마나 필요한가」를 **자가 서로 다른 지분 축 3개**(필요 기술=남은 난제 총량 / 핵심 광물=원재료비 / 전문가=인력 병목 총량)로 분해해 축마다 100% 누적 가로 막대 1줄 + 항목 목록(지분·근거 1문장·선도 업체 칩·산지·채굴사 칩)을 세로로 쌓는다. **세 축은 분모가 달라 합칠 수 없으므로** 축마다 「기준」 문구가 상시 노출되고 세 축을 합친 단일 지표는 없다. 해부에 등장하는 업체 칩에는 내 보유·관심 종목이 「◆ 보유 · ◇ 관심」 기호로 표시되고(항목마다 걸린 개수 배지 + 상단 「이 기술에 걸린 내 종목」 요약, 개수만 세고 지분은 섞지 않음), 그 대조는 업체를 붙이는 필요 기술·핵심 광물 축에서만 성립하며 조회 실패·대조 대상 없음은 「없음」이라 말하지 않고 요약을 감춘다. 진입은 목록 카드의 「해부」 링크와 리포트 상단 상호링크 2곳이며(nav 신규 항목 없음), 미기입 기술은 백지가 아니라 「아직 해부되지 않았습니다」 안내와 리포트 링크를 보여준다(자동 수집 소스가 없어 전량 루틴 기입이라 배포 직후엔 대부분 비어 있다) |

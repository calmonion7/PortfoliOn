...
- **상시 게이트: `backend/tests/test_no_public_reads.py`**(task#233에서 pytest로 승격 — 이제 회귀는 `pytest`가 자동으로 막는다). AST 파싱이 아니라 **라이브 `app`의 라우트를 돌며** 각 엔드포인트 함수 default가 4개 인증 의존성(`get_current_user`·`get_current_user_or_api_key`·`require_admin`·`require_admin_or_api_key`) 중 하나인지, 그리고 라우트 수준 `dependencies=[...]`도 함께 검사한다(데코레이터·주입 실수까지 포착). 허용목록 `ALLOWED_PUBLIC` 9개를 **exact-match 양방향**으로 단언하므로 새 무인증이 생기면 실패하고, 목록에 있는데 인증이 걸렸거나 사라졌으면도 실패한다(`test_api_doc_sync.py`의 `KNOWN_UNDOCUMENTED`와 동형·self-maintaining). 새 공개 엔드포인트를 넣으려면 **이 ADR을 먼저 개정**해야 한다.
- ⚠️ **라우트 트리 형태는 FastAPI 버전에 따라 다르다** — 배포 이미지(0.138.1)는 include된 라우트를 `_IncludedRouter`로 감싸 `.path`도 `.routes`도 숨기고 `original_router`만 주므로, 평탄 `app.routes` 순회는 **라우트 0개를 세며 조용히 통과**한다(로컬 0.128.8에선 138개가 나와 발산이 은폐된다 — task#233 라이브 프로브 실측: 헬퍼 138 vs 평탄 0). 순회는 **공용 헬퍼 `backend/tests/_routes.py:walk_routes()` 한 곳**으로 모았고 신규 게이트와 `test_api_doc_sync.py:_live()`가 함께 쓴다. `requirements.txt`가 핀 없는 `fastapi>=0.104.0`이라 이 발산은 계속 진행되므로, 라우트를 열거하는 새 테스트도 반드시 이 헬퍼를 거칠 것.
- 선행 스캐폴딩이던 `scripts/audit_unauth_endpoints.py`는 **커밋하지 않는다**(정본이 pytest로 옮겨졌고 두 사본을 두면 drift한다) — 컨테이너 ad-hoc 프로브 용도로 로컬에만 남긴다.

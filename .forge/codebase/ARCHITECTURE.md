...
1. **앱 진입 — `backend/main.py`**
   ...
   - `app.include_router(...)`로 라우터를 마운트. auth 라우터가 먼저, admin 라우터가 마지막.
   - `/health` (`GET`/`HEAD`) 헬스체크.
   - `lifespan`: 기동 시 `_migrate()` → `sched.start()` → `_warm_market_cache()`(데몬 스레드), 종료 시 `sched.stop()`.
...
2. **라우터 — `backend/routers/`** (모두 `APIRouter`)
   ...
   - 라우팅 함정(구체 경로 먼저 등록): `PUT /api/stocks/enrich/batch`는 `PUT /api/stocks/{ticker}/enrich`보다 **먼저**. 마찬가지로 `GET /api/portfolio/exposure`·`GET /api/portfolio/rebalance`·`PUT /api/portfolio/rebalance/targets`는 `PUT/DELETE /api/portfolio/{ticker}`보다 **먼저** 등록해야 고정 경로가 ticker로 라우팅되지 않는다.
...
3. **서비스 — `backend/services/`**
   ...
   - **순수 계산 서비스**: 일부 서비스는 DB/외부호출 없는 순수 함수다. `services/rebalance.py`(`compute_rebalance` + 공유 헬퍼 `value_holdings_krw`)와 `services/exposure.py`(`compute_exposure`)가 대표(아래 리밸런싱·노출 흐름) — 테스트 용이, 라우터가 데이터를 모아 넘기고 결과만 sanitize해 반환.
...
- `BATCHES` = 28개 배치의 정적 메타데이터 리스트. `get_batch(job_id)`로 조회(내부 `_BY_ID` dict).
- 각 항목의 핵심 속성:
...
- 시장별 분리: 일일 리포트 `daily_report_kr`(20:30 KST)·`daily_report_us`(07:00 KST, ADR-0012), 실적 `earnings_kr`/`earnings_us`, 월간 `monthly_kr`/`monthly_us`, 섹터 `kr_sector_fetch`/`us_sector_fetch`, 랭킹/추천도 KR/US 쌍. 공통(시장 무관): `dividend_fetch`(주 일 05:00), `beta_fetch`(주 일 05:30, task#150) 등.
...
- 히스토리(매물대/RSI/베타): `get_history_df` — KR은 키움(ka10081/82/83) 우선 → yfinance 폴백. `get_financials`/`get_annual_financials`/`get_analyst_data`도 KR/US 분기.
...
## 리밸런싱 계산 흐름 — `backend/services/rebalance.py` (task#146/#147/#150)

- **공유 헬퍼** `value_holdings_krw(holdings, usdkrw)` — 보유를 전체-포트 KRW로 환산해 `(rows, full_total, fx)` 반환. rebalance·exposure 두 계산기가 공유(eco: 중복 방지). `_finite_float`로 None/Decimal/비유한값 정규화, `usdkrw≤0`/None이면 US 보유를 `no_fx`(총계·비중서 제외), KR은 fx=1.0. 가격/수량 무효는 결과에서 제외.
...
- **저장** (`backend/services/storage/portfolio.py`): `set_target_weights`(None→NULL 배치 UPDATE). `save_holdings`의 UPSERT는 `target_weight`만 `COALESCE(EXCLUDED, 기존값)`로 preserve-on-null.
- **프론트** (`frontend/src/pages/RebalanceTab.jsx`): Portfolio 분석 탭의 "리밸런싱" 서브탭.

## 노출·집중도 흐름 — `backend/services/exposure.py` (task#149/#150)

- **순수 함수** `compute_exposure(holdings, quotes, fx, sector_map, beta_map=None)` — DB/외부호출 없이 보유 종목의 통화(시장)·섹터·단일종목 3축 쏠림을 전체-포트 KRW 환산 비중으로 계산. KRW 환산·전체-포트 분모는 `rebalance.value_holdings_krw`를 재사용. 반환: `holdings`(비중 내림차순)·`currency`·`sector`·`concentration`(top3/top5/max_single)·`warnings`(single_name>25%, sector>40%)·`portfolio_beta`·`beta_coverage_pct`·`beta_covered_count`·`beta_missing`·`no_fx`.
- **포트폴리오 베타**(베타가중 노출): `beta_map`에 있는 종목만 비중가중 평균으로 `portfolio_beta` 계산(coverage 비율도 반환). 저장 베타(`stock_beta`)가 없는 종목은 계산서 제외.
- **라우터** (`backend/routers/portfolio.py`): `GET /api/portfolio/exposure`(auth, 보유(holding)만 대상). 보유목록 + `get_quotes_batch` 시세 + 저장 FX(`_usdkrw_rate`) + 섹터맵(US=`snapshots.data->>'sector'` 최신, KR=`kr_sector_service.map_holdings_to_sectors` 저장 인덱스) + `stock_beta` 저장 베타맵을 `compute_exposure`에 넘기고 `sanitize`해 반환(요청경로 라이브 베타/키움 호출 없음).
- **프론트** (`frontend/src/pages/ExposureTab.jsx`): Portfolio 분석 탭의 "노출" 서브탭(섹터/매크로/상관/리밸런싱 옆).

## 베타 트래킹 흐름 — `backend/services/beta.py` (task#150)
...
- **pages** (`frontend/src/pages/`): 라우트 레벨 + 허브 탭. `Research`(리포트·추천·랭킹·다이제스트·캘린더 탭 컴포지션), `MarketHub`(시장지표·수급지표), `Portfolio`(대시보드 탭 + 분석 탭: 섹터/매크로/상관관계/리밸런싱/노출 서브탭). 각 허브가 하위 페이지(`Reports`·`Ranking`·`Recommendations`·`Digest`·`Calendar`·`Market`·`Analytics`·`SectorTab`·`MacroTab`·`RebalanceTab`·`ExposureTab`)를 탭으로 렌더.
...
| 기동 마이그레이션 | `backend/main.py` (`_migrate`) |
...
| 배치 메타데이터 | `backend/services/batch_registry.py` (`BATCHES`, 28개) |

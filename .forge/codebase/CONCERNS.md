...
- **uuid 컬럼 `= ANY(%s)`에 파이썬 str 리스트 → text[]가 돼 `operator does not exist: uuid = text` 라이브 즉사.** 단건 `WHERE user_id = %s`는 암묵 캐스트로 동작하던 게 배열화에서 깨짐. **`ANY(%s::uuid[])` 명시 캐스트 필수.**
  - 주의: **text/varchar 컬럼의 `= ANY(%s)`는 안전**(암묵 text[] 매칭). 예: `backend/routers/portfolio.py`의 `SELECT ... FROM stock_beta WHERE ticker = ANY(%s)`는 `ticker`가 text라 무해. uuid 컬럼(`user_id` 등)에만 캐스트가 필요.
...
- **✅ 세 소비처 모두 가드됨**:
  ...
  - `backend/services/report_generator.py` `generate_report` KR beta — **이번(86b714e, task#152 후속 fg-quick)에 통일됨**: `ks11_ret`뿐 아니라 `_daily_returns`(daily_df 파생)도 tz-strip(162-166행). 키움→yfinance 폴백으로 daily_df가 tz-aware가 돼도 concat이 안 깨진다. **더는 잔존 갭 아님**(이전 맵의 §1d/§5c "쌍둥이 잔존 갭"은 해소).
...
- **뉴스도 dual-path(신규, task#152)**: 리포트 상세는 `/api/stocks/{ticker}/news` **라이브 fetch**(`scraper.get_news`) + **스냅샷 폴백**, 다이제스트 `_recent_news`는 스냅샷(`_latest_snapshots`)만 읽어 종목당 top2(스크레이프 0). 저장·라이브 경로가 갈리므로 뉴스 형태/필드 변경 시 둘 다 확인. `backend/services/scraper.py`(`_dedup_sort_limit` 10건 dedup/최신순)·`backend/services/digest_service.py`.
...
- 대응 위치:
  ...
  - `backend/services/utils.py` `sanitize` — NaN/inf→None 안전망. `_build_all`·exposure 반환·F&G(`sentiment.get_fear_greed`)를 감쌈.
  - `backend/services/beta.py` `_fin_num` — `math.isfinite` 통과 못하면 None(저장 안 함) → `stock_beta.beta`에 NaN 유입 차단. `sentiment.py` `_num`도 동일(`math.isfinite` 통과 못하면 None).
...
1. **풀 경합(cold-start)** — 콜드 첫 호출에 워커 ThreadPool×카드당 다중 DB read가 풀 경합(PoolError)→throw→500→프론트 `usePortfolioData.fetchDashboard` catch가 silent로 삼킴→빈 그리드. 헤더(`/api/portfolio`=단일 쿼리)는 N 정상이라 "헤더 N·그리드 빈"(task#102). `_build_all`의 ThreadPool은 `max_workers=min(len(holdings), 10)`(`stocks.py`).
...
- **일반화**: 저장값/배치-백킹 기능(랭킹·KR 업종 모멘텀·포트 베타·시장지표 캐시)은 **백필/재생성 전 빈값**이 정상 동작(graceful). "값이 안 나온다"를 코드 버그로 단정하기 전에 저장소에 값이 실제로 있는지 프로브(§15).
...
## 6. 외부소스 stale 폴백 & 취약 엔드포인트 (task#151·신규)
...
- **대응: 취약 외부소스는 VIX식 수동 폴백을 각자 구현할 것.** 정합 패턴 2종:
  ...
  - `backend/services/market_indicators/sentiment.py` `get_fear_greed` — `_get_cache` → `_fetch_fear_greed()` 성공 시 저장+반환, **실패(None) 시 `_mc_load` 직전 저장값 반환**, 그것도 없으면 None. `get_or_refresh`를 **의도적으로 안 씀**(commit 6cd9d5c: "get_or_refresh 대신 VIX 수동 폴백").
- 신규 외부소스 지표를 붙일 때 `get_or_refresh`만 믿고 stale 폴백을 생략하면, 소스 일시장애 시 값이 통째 사라진다(있던 저장값이 있어도).

### 6b. CNN F&G 비공식 엔드포인트 봇차단 취약성
- `backend/services/market_indicators/sentiment.py` — `https://production.dataviz.cnn.io/index/fearandgreed/graphdata`는 **CNN 비공식(문서화 안 된) 엔드포인트**로, 전체 브라우저 헤더(User-Agent·Origin·Referer·sec-ch-ua 등 `_CNN_HEADERS`)를 실어 우회 fetch한다. **CNN이 봇차단(헤더 검사 강화·엔드포인트 이동·차단)하면 fetch가 조용히 실패**할 수 있다(→ §6a 수동 폴백이 직전 저장값으로 graceful, 없으면 None). 요청경로 증분·**배치 없음**(`GET /api/market/fear-greed`, `batch_registry` 무등록). US 전용. FRED 등 공식 소스가 아니므로 계약이 불안정 — 값이 stale/None으로 굳으면 이 취약성부터 의심.
...
- **최근 준수 사례**: `stock_beta` 테이블은 `app_schema.sql` + `main.py` `_migrate`(`CREATE TABLE IF NOT EXISTS stock_beta`)에 **쌍으로** 추가됨(task#150, dividends 패턴 미러). 이 쌍 규율이 정착 중.
...
- **배치-백킹 뷰(랭킹·KR 업종 모멘텀·베타·노출)는 외부 API를 요청·기동 경로에서 라이브 호출 금지** — 배치가 사전계산해 `market_cache`/테이블(`stock_beta` 등) 저장, 요청은 저장값만 read(요청당 N콜 직렬=수초 지연, task#50). `compute_exposure`도 순수함수(DB/외부 호출 0)로 `beta_map`/`sector_map`을 인자로 받는다.
  - 예외(의도된 요청경로 증분): fx/vix/commodities/indices/**fear-greed**는 배치 없이 요청경로에서 증분 fetch(TTL캐시→`_mc_load`→라이브→저장). §6a stale 폴백 규율이 이들에 적용.
...
- **배치 ThreadPool 워커 ≤ DB 풀 사이즈**(수급 스크리닝 교훈) — 대시보드 콜드 풀 경합(§3)의 근원. DB 풀은 `backend/services/db.py` `ThreadedConnectionPool(minconn=1, maxconn=20)`(psycopg2 풀은 소진 시 블록이 아니라 PoolError를 던지므로 최대 워커 수 이상으로 둠). ⚠️ 정보성: `stocks.py` 이름-백필 경로의 주석은 아직 `maxconn=10`으로 표기(코드 실제 `maxconn=20`과 stale) — 판단 근거로 삼지 말 것.
...
- 이 앱은 `--up`=빨강(상승)·`--down`=파랑(하락, `frontend/src/styles/tokens.css`)이라 `.badge--success`=빨강·`.badge--danger`=파랑(`frontend/src/components/ui/Badge.css`). **의미 상태 배지(수급 밴드·노출 경고·F&G 등)에 success/danger 쓰면 KR 가격색으로 박혀 Western 의도와 반전**(수급 배지 우호=빨·경계=파 버그, b288f394). 의미 배지는 `frontend/src/components/ui/SupplyBadge.jsx`처럼 전용 색 명시. 노출/베타 경고 배지·F&G 게이지(`FearGreedSection.jsx`, CSS 그라디언트+전용색)도 caution/전용색 사용(task#149·#150·#151). `warning` 변형은 토큰 미정의로 현재 깨져 있음.
...
- **report_generator KR beta tz-strip 쌍둥이 갭 해소**(86b714e, task#152 후속 fg-quick, 2026-07-07): `generate_report`가 `ks11_ret`만 벗기고 `_daily_returns`(daily_df 파생)는 안 벗겨 키움→yfinance 폴백 시 concat TypeError→조용히 None 되던 것을 `beta.py`와 동일 가드로 통일. 이전 맵의 §1d/§5c "잔존 갭"은 더는 유효하지 않음(§1d에 통일 사실 기록).

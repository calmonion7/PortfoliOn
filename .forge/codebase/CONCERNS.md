...
| **B78** | `uat282`의 「시장 규모 추정치」 **상세 축 13개가 실행 전 모드로 게이트돼 라이브 렌더를 영구 스킵**한다 —  **근거 주석**이 「라이브 7 slug 전부 `market.estimates` 키가 없다(2026-08-21 재확인)」를 근거로 13축을 `if (R.mode === 'inject')`에 가뒀는데, **postgres 직접 조회 결과 15종 전부 `estimates`가 채워져 있다**. 주석의 근거가 거짓이 됐고 그 13축은 픽스처만 재고 라이브를 전혀 안 재는 상태로 **조용히 전환**됐다 — 프로브는 계속 초록이고 단언 총계도 줄지 않으므로 **어떤 자동 신호도 알리지 않는다**. **task#317의 정확한 재발**(같은 파일·같은 클래스: 주석이 코드 경로를 가두면 그 주석은 테스트 없는 코드다). 처방: 게이트를 **런타임 데이터 판정**으로(`if (estimates?.length)`) — 모드가 아니라 데이터를 조건으로 삼으면 데이터가 진화해도 따라간다. 「표본 0 — 미검증」을 PASS로 세지 말 것(task#318) | `scripts/uat282-tech-structure.mjs` (`RUNS` 루프의 `if (R.mode === 'inject')` , else , 근거 주석 ) | 발행물에 `market.estimates`가 채워진 지금(=항상) |
| **B79** | `uat298`의 real 모드가 **이제 거짓인 전제로 하드 분기**해 지금 돌리면 **거짓 FAIL 24건**을 낸다 — `if (R.mode === 'real')` 진입 즉시 `absent-variants`·`absent-watch-items`를 **무조건 `ABSENT` 기대값**으로 단언하고 `continue`해 그 뒤의 상세 렌더 검증 블록(inject 전용)에 **영원히 도달하지 못한다**. task#322가 7종을 발행하며 그 두 필드를 채워 전제가 거짓이 됐다. **실측**: `screenshots-uat298/result.json`(2026-08-21 06:59, `control:null`) 집계 = **단언 284건 중 FAIL 24건**, 전부 `absent-variants` 8 · `absent-watch-items` 8 · `absent-toc-chips` 8이고 메시지가 `got="PRESENT(회귀!)" want="ABSENT"`다. **이 프로브의 exit 코드는 게이트로서 사망했다**(「항상 FAIL하는 축은 게이트가 아니다」) — 24건 상시 FAIL이 진짜 회귀를 묻는다. ⚠️ `.forge/retro/260819-190100-tech-anatomy-2of2.md`가 **이 실패양상을 이미 관측**했으나 같은 파일의 학습 절에서 「승급하지 않음」으로 처리했고 그 미승급이 이 24건을 만들었다(가토 ⑥). 처방: 전제를 런타임 데이터 판정으로. 그리고 「real에서 NULL이어야 한다」류 **데이터 상태 단언은 발행 파이프라인이 그 데이터를 채우는 순간 반드시 거짓이 되므로** 애초에 두지 말고 「렌더가 데이터와 일치하는가」로 쓸 것 | `scripts/uat298-tech-structured.mjs` | 지금 그 프로브를 real 모드로 실행하면(=항상) |

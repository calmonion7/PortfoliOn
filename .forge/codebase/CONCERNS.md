...
**Location:** `backend/routers/stocks.py:382-388` (the dividend-income computation inside the card builder).

**The bug (root cause of the task#102 symptom):** `avg_cost` / `quantity` come from a PostgreSQL **NUMERIC** column and arrive as Python `Decimal`; `annual_dividend_per_share` (`annual_div`) comes from the dividends store as a Python `float`. Dividing them directly (`annual_div / avg_cost`) raises `TypeError: unsupported operand type(s) for /: 'float' and 'decimal.Decimal'`. That throw happens *inside per-card enrichment*, so the `_safe` wrapper (`backend/routers/stocks.py:494`) catches it and falls back to `_minimal_card` (`backend/routers/stocks.py:465`) — meaning **every holding card silently loses its enrichment** (consensus/dividend/supply/insider all stripped to the minimal shape). The grid renders but is empty of analysis.

**Fix (in place — do not regress):** coerce both operands with `float(...)` before arithmetic — `round(float(annual_div) / float(avg_cost) * 100, 2)` and `round(float(annual_div) * float(qty), 2)` (`backend/routers/stocks.py:385-388`), with the explanatory comment at `backend/routers/stocks.py:382-384`.

**Why it's a standing class, not a one-off:** the `_minimal_card` fallback **masks the root cause** — the response is a valid 200 with valid-shaped (but degraded) cards, so there is no 500 and no obvious error. Diagnose via the per-card fallback log: `docker logs portfolion-backend-1 | grep '최소카드 폴백'` (the `print(...최소카드 폴백...)` at `backend/routers/stocks.py:499`). **Generalize:** any arithmetic mixing a NUMERIC-column value (→`Decimal`) with an external/computed `float` (yfinance, dividends, FX, consensus) is a fresh instance — coerce at the boundary. This is the *same* fragility family as the §3 NaN/inf trap (external float meets a strict consumer), just surfacing as a TypeError instead of a serialization 500.

---

## §2. yfinance percent fields are *decimal fractions* — ×100 display scale (recurring: #122 short%, #123 pct_buy)

**Mechanism:** yfinance returns percent-like fields as **0–1 decimal fractions**, not percents: `info.shortPercentOfFloat` (0.0098 = 0.98%), `institutional_holders.pctHeld` / `pctChange`, `insider_purchases` `% Buy Shares` / `% Sell Shares` (0.0476 = 4.76%), `info.dividendYield`, etc. The backend stores them raw (`backend/services/us_supply.py:78,81,119,131` — `_finite(...)` of `% Buy Shares` / `% Sell Shares` / `shortPercentOfFloat` / `pctChange`) and the **frontend must multiply by 100 for display**.
...
**Location:** pool config `backend/services/db.py:16-28`; dashboard build `backend/routers/stocks.py:484-506`.

The dashboard endpoint fans `holdings` cards across a `ThreadPoolExecutor(max_workers=min(len(holdings), 10))` (`backend/routers/stocks.py:502`), and each card does multiple DB reads (snapshot/consensus/dividend/supply-demand/insider). On a cold first call the pool can be starved: psycopg2's `ThreadedConnectionPool` **throws `PoolError` on exhaustion rather than blocking**, so an exhausted pool surfaces as a 500, not a queue.
...
- Per-card `_safe` wrapper (`backend/routers/stocks.py:494-500`) — any card that throws falls back to `_minimal_card` instead of 500-ing the whole response (invariant: holdings=N → always N cards, task#102). NOTE: this *also* masks the §1 TypeError class — see §1 for the diagnosis log.
- Whole-response `sanitize(...)` net (`backend/routers/stocks.py:506`) against NaN/inf (see §4).
- The batch-quote call is wrapped try/except→`{}` (`backend/routers/stocks.py:486-490`; now `print`s "일괄시세 실패" instead of swallowing silently — task#127/#128).
...
- `_usdkrw_rate` source guard: `backend/routers/stocks.py:322-337` — `math.isfinite` check (line 337) so a NaN FX returns `None` (NaN≠None would otherwise slip past the `if fx is None` guard in `_portfolio_totals` and poison `totals`, task#104).
- `services.utils.sanitize` (`backend/services/utils.py`) recursively maps NaN/inf→None; applied as a net in `_build_all` (`backend/routers/stocks.py:506`) and in report-bake writes (`backend/services/report_generator.py:328,497`).
- Market-index/CAPE crawl: `math.isfinite` checks throughout `backend/services/market_indicators/indices.py:25,35,83,89` + `sanitize` net at `backend/services/market_indicators/indices.py:137-138` (task#113; CAPE crawled from multpl.com, see §5).
...
**DART `fnlttSinglAcntAll` (KR financials)** — `backend/services/market/kr.py:373-509`:
- `fs_div` is a **required request param** (omitting it → DART `status 100 "필수값 누락"` → all None); request CFS→OFS fallback (`backend/services/market/kr.py:462-470`).
- The fs_div-requested response **does not echo per-row `fs_div`**, so filtering rows by `row.get("fs_div")` skips everything — `_dart_extract_3y` must *not* filter (`backend/services/market/kr.py:380-395`, comment at `:382`).
- Match by XBRL `account_id` (`backend/services/market/kr.py:384-386`), e.g. interest-coverage denominator is `이자의 지급` = `ifrs-full_InterestPaidClassifiedAsOperatingActivities` (`_DART_INT`, `backend/services/market/kr.py:376`), **not** `금융비용`. (Contrast `fnlttSinglAcnt` in `backlog.py` which is the opposite convention.) Bug found only in live UAT — retro `.forge/retro/2026-06-27-financial-health-metrics-2of2.md` (task#117). A `silent except` had been *hiding* this exact bug — the `get_annual_financials_kr` broad excepts (`backend/services/market/kr.py:502,507`) now `logger.warning` (task#127/#128, commit `40662f15`) instead of silently returning `[]`.

**yfinance label conventions** — `backend/services/market/us.py` + `_yf_val` (exact-match, returns None on mismatch):
- `get_income_stmt()`/`get_balance_sheet()`/`get_cashflow()` *methods* use no-space index labels (`OperatingCashFlow`); `.income_stmt`/`.cash_flow` *properties* use space labels (`Operating Cash Flow`). Mixing them → silent None for FCF/CapEx. Must use the `get_*()` methods (`backend/services/market/us.py:15-16,27`; task#117). The cashflow/FCF/consensus fetch excepts (`backend/services/market/us.py:30,81,103,153,158`) now `logger.warning` instead of swallowing (task#127/#128).
...
**S&P 500 Shiller CAPE crawl** — `backend/services/market_indicators/indices.py:42-108`:
- FRED has **no** CAPE series (its "Case-Shiller" is the *housing-price* index), so CAPE is scraped from `multpl.com/shiller-pe` via `requests.get` + `BeautifulSoup(html, "html.parser")` (`backend/services/market_indicators/indices.py:45,98-106`). `html.parser` (not `lxml`) is required because the local `.venv` lacks `lxml` (`CLAUDE.md` Gotcha; Docker has it, local doesn't). HTML-structure drift on multpl.com would break parsing — both `_parse_multpl_cape` and the fetch were previously `except Exception: return None` (silent) but now `logger.warning(...)` before the `None` return (`backend/services/market_indicators/indices.py:93-94,107-108`; task#127/#128), so a layout change degrades gracefully **and is now logged** (CAPE still just disappears from the UI, but the failure is diagnosable).
...
- `_corroborated_pick` (`backend/services/market/kr.py:131`) — 2-of-N majority across independent price feeds; a feed is trusted only if ≥1 *other* feed agrees within 2x ([0.5, 2.0]).
- `_kr_pick_basic` (`backend/services/market/kr.py:185`) — lazy escalation: normally Kiwoom NXT + Kiwoom KRX (2 calls); on disagreement it escalates to KIS + Naver (up to 4 feeds) and discards the outlier.
- Degenerate fallback `_kr_pick_degenerate_lazy` (`backend/services/market/kr.py:166`) — when only one feed is available, falls back to single-feed self-check (`_price_sane`, prev_close ±30% / daily-bar 2x at `backend/services/market/kr.py:113`).
- (`_kr_pick_regular` at `backend/services/market/kr.py:144` is the report-snapshot path — no majority vote, prev±30% + daily-bar 2x only, task#94/95.)

**Guard B — report-bake independent-feed gate (KR only).** `backend/services/report_generator.py:285-325`:
- Before baking, cross-checks the KRX `price` and daily-bar reference close against an *independent* ref feed at 2x. Multi-feed ref (task#118): Naver retry-once (`backend/services/report_generator.py:298-307`) → KIS fallback (`backend/services/report_generator.py:309-313`, dormant-safe if key unset).
- **If no ref feed at all → bake is SKIPPED** (prior good snapshot kept, wrong<missing) with a loud `print("[Report] ...")` (`backend/services/report_generator.py:315-318`). This closed the earlier hole where "no Naver → skip verification, proceed" silently no-op'd the gate (task#118, 005930 70k recurrence).

**Why it stays fragile (document, don't "fix" naively):**
- Guard B is **not immune to KRX self-consistent contamination** — if both KRX TRs (quote ka10001 + daily-bar ka10081) glitch together at the same batch instant, they corroborate each other and `_price_sane` is blind; only the independent ref feed catches it (task#101, glitch-detect `print` at `backend/services/report_generator.py:323`).
...
- The whole majority-vote machinery (`backend/services/market/kr.py` is ~15 `except` blocks deep) is intricate enough that a careless edit to feed priority can silently regress (e.g. the task#96 KRX single-anchor false-reject that survived into task#98). NOTE: as of task#127/#128 the previously-silent broad excepts in this file now `logger.warning` (9 sites), so a swallowed feed-fetch failure is at least diagnosable — but control flow (fallback ordering) is byte-identical.

---

## §7. FOMC `_FOMC_DATES` — static hard-coded list, yearly manual refresh, exhaustion → graceful missing

**Location:** `backend/routers/calendar.py:27-35` (the comment + list), `backend/routers/calendar.py:201-202` (consumed in `_get_econ_events`, which `_get_events` calls at `:99`).

The FOMC policy-decision calendar (task#125) is a **static Python list** of hard-coded dates (`2025-…` … `2027-12-08`), expanded unconditionally into calendar events regardless of `FRED_API_KEY` (`backend/routers/calendar.py:201-202`, comment at `:195`). Two concerns:

1. **Yearly manual update required.** Coverage ends at ~2027-12; the comment at `backend/routers/calendar.py:27` flags this ("차기 일정 공표 시 연 1회 수동 갱신; 소진 시 FOMC 미표시(graceful)"). When the list is exhausted the FOMC events simply stop appearing — **graceful missing, not a crash** (the loop just iterates an empty tail). The failure mode is "calendar silently loses a recurring event class," which nobody notices until someone looks for a future FOMC date.
...
**Impact:** the KR earnings-calendar feature (task#121) relies on `t.calendar["Earnings Date"]` (`backend/routers/calendar.py:115-117`) via `_yf_sym` (called at `backend/routers/calendar.py:83`); `.KQ` 404s mean some KOSDAQ holdings show no forward earnings date in the calendar. yfinance is the *only* forward KR earnings-date source (Naver/DART provide no forward dates), so there is no fallback. Documented as a known limitation in retro `.forge/retro/2026-06-28-event-calendar-expansion-3of3.md` (D3) and `CLAUDE.md` Gotchas. The empty-`exchange`→`.KS` default is "existing app behaviour, not a new risk" per that retro, but it remains a latent wrong-symbol source for KOSDAQ.

---

## §9. Broad `except` swallowing — largely DE-SILENCED (task#127/#128), residue remains

**MAJOR UPDATE (commit `40662f15`, task#127+#128):** a *de-silence sweep* addressed most of this class. ~77 broad `except Exception:` blocks across **28 backend files** — which previously swallowed silently (`pass` / `return default`) — now emit a `logger.warning(f"...: {e}")` immediately before their fallback. The sweep was **LOG-ONLY**: control flow is byte-identical (no return/fallback/structure changed), pytest stayed 982 green. 27 files that lacked it gained `import logging` + `logger = logging.getLogger(__name__)`. The project's own "silent except 금지" rule (repeatedly re-learned — a silent except *hid* the DART fs_div bug in §5, retro `.forge/retro/2026-06-27-financial-health-metrics-2of2.md`) is now enforced across these files.

**The 28 de-silenced files (verified):** `routers/{analytics,batches,events,report,stocks}.py`; `services/{analysis_service,consensus_pipeline,digest_service,indicators,investor_service,report_generator,scraper,short_sell_service,us_supply}.py`; `services/kis/quote.py`; `services/market/{__init__,kr,us}.py`; `services/market_indicators/{cache,commodities,earnings,econ,exports,fx,indices,macro}.py`; `services/storage/{dates,names}.py`. Verified counts, e.g.: `backend/services/market/kr.py` = 9 `logger.warning`, `backend/services/report_generator.py` = 9, `backend/services/market_indicators/indices.py` CAPE excepts (`:93-94,107-108`) now log, `backend/services/us_supply.py` per-fetch excepts (`:48,115`) now log. It is **no longer accurate to call these "silent"** — a swallowed external-fetch/parse failure in any of these files is now diagnosable from the logs.

**What REMAINS silent (NOT touched by the sweep):**
- **Frontend silent catches** (untouched — sweep was backend-only): `frontend/src/hooks/usePortfolioData.js` (`fetchDashboard` `catch { // silent }`, `refreshLivePrices`, and `.catch(() => {})` swallows). The dashboard silent-catch is the one that produced the "header N / empty grid" symptom (task#102) — backstopped by the §3 bounded retry, but the catch itself is unchanged. Additional `.catch(() => {})` in `App.jsx`, `Calendar.jsx`, `Ranking.jsx`, `ReportManualGen.jsx`, `ConsensusSettings.jsx`, `AdminAnalytics.jsx`, `GuruCrawlNow.jsx`.
- **`backend/main.py:35-36`** — `_warm_market_cache` startup warm still `except Exception: pass` (silent). `main.py` was **not** in the 28-file list; its `_migrate` blocks (`:44,49,…`) already `print` on failure, but this one startup swallow remains. (The older `_warm_calendar_cache` startup-FRED warm was **REMOVED** in commit `f1d9658e` — no longer a concern.)
- **Intentionally-narrow excepts** left as-is by design: `except (ValueError, TypeError, KeyError)` blocks (e.g. `_dart_extract_3y` int-parse at `backend/services/market/kr.py:393`) were deliberately not touched — they catch a specific expected condition, not arbitrary failure.
- **Retry / re-raise blocks** left as-is: `backend/services/db.py:37-39` — `get_connection` rolls back then **re-raises** (correct, not swallowing); retry-once loops (e.g. Naver ref retry in `report_generator`) and token-reissue-on-401 in the Kiwoom/KIS clients re-raise or retry rather than swallow.
- **`backend/services/job_runs.py`** — job-run telemetry recording swallows (acceptable by design: telemetry must not break the job; not in the 28-file list).
- **`backend/services/guru_scraper.py`** — external-fetch swallows (not in the 28-file list; still silent).

**Batch-fetch anti-pattern (documented in `CLAUDE.md`):** batch-backing views (ranking, KR sector momentum) must **not** swallow fetch failures silently and must **not** bake empty/all-None results into the cache (task#48 `_fetch_one_sector` swallowed empty closes → all-None baked; established task#48→49→50). Guard the *failure class* (all-None) not a suspected trigger. (Independent of the task#127/#128 log-only sweep — the anti-pattern is about *baking* bad results, which the sweep did not change.)

---

## §10. Other fragile / debt spots

- **`_get_econ_events` is an inline cache-miss FRED fetch, not batched** (`backend/routers/calendar.py:193-197` docstring; live FRED call at `:219-222`) — the docstring flags the upgrade path: "fetched inline at cache-miss; upgrade path = batch into market_cache if FRED rate-limits ever become a problem." Every cold calendar month-miss hits live FRED. (NOTE: the *startup* FRED-on-boot defect via `_warm_calendar_cache` was removed in `f1d9658e`; this remaining inline-on-cache-miss path is the lower-severity residue.)

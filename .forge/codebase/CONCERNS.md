...
PortfoliOn의 LIVE 관심사항을 구체 파일경로와 함께 정리한다. 근거는 `CLAUDE.md` gotchas 절, `.forge/adr/`, `.forge/retro/`, 그리고 실제 코드 재확인. 각 항목은 "왜 위험한지 + 어디를 봐야 하는지 + 대응"으로 구성. (이번 재매핑 핵심 갱신: **task#170 — "005930 정확히 70000.0" 박제의 실제 원인이 피드 글리치가 아니라 로컬 pytest→prod DB 오염이었다는 귀속 정정**(§1f·§4), task#169 **fixture-writes-live**(역방향 fixture 가족) + conftest `_block_real_db` 가드 신설, 상대 밸류에이션 KR EV/EBITDA 소스 리스크(ADR-0024, §1g), 디자인 리뉴얼 5/5(task#171~178, ADR-0025) 완결에 따른 배포 footgun 신규 항목(§7d) 및 색 관례 후속(§14), kr-exports 요청경로 stale 재조회 해소(§17).)
...
- **✅ 정본 예시(요청경로 — kr_exports, task#176)**: `backend/services/market_indicators/exports.py` `get_kr_exports` — in-memory 캐시 미스 → `_mc_load` 저장값 있으면 `_set_cache`로 24h TTL **워밍 후 반환**, 저장값도 없을 때만 라이브 fetch. 예전엔 저장값을 워밍 없이 매 요청 스킵해 라이브 재조회(3.75~7초)가 상시 발생하던 stale 분기가 있었으나 제거됨(고아가 된 `_exports_is_stale` 헬퍼도 동반 삭제). fx.py 패턴과 동형.
...
- **⚠️ DB `execute`를 부르는 함수의 테스트는 execute를 patch할 것** — 로컬 pytest가 라이브 Docker postgres에 붙을 수 있어, `clear_cache` 테스트가 **라이브 `calendar_cache`에 실 DELETE**를 내보내던 것을 task#166 리뷰(HIGH)가 포착해 patch로 차단(`backend/tests/test_calendar_cache_invalidation.py`). **이 규율은 이제 conftest `_block_real_db` autouse 가드(§1f, task#169)로 프로젝트 전역에 강제된다** — 신규 DB-write 함수 테스트가 이 가드에 raise로 걸리면 mock을 추가할 것(가드 자체를 풀지 말 것).

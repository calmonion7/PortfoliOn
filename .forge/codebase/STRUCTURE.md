...
├── requirements.txt          fastapi·uvicorn·apscheduler·yfinance·pandas·psycopg2·authlib·jose·bcrypt·lxml… (anthropic 없음)
...
├── tests/       (~130 파일)  pytest (conftest.py에 _block_real_db autouse 가드)
├── data/                     정적 유니버스 + 일부 파일 캐시 + 레거시 잔존물(§6)
...
| `services/market_indicators/` | `cache.py`(market_cache I/O) `fx` `commodities` `earnings` `econ` `exports` `macro` `indices` `sentiment` `kospi_futures` `kospi_signal` | `__init__`이 `get_*`/`_fetch_and_save_*` 공개 |
...
|---|---|---|---|
| `routers/auth.py` | `/api/auth` | 10 | register·login·refresh·logout·me·oauth/{google,github}(+callback)·oauth/token |
...
| `routers/report.py` | `/api` | 26 | report/{progress,list,generate,generate/{ticker},backfill}·`{ticker}/history`·backlog·disclosures·agm·insider-trades·us-supply·**catch-all `GET /api/report/{ticker}/{date_str}`**·consensus/{batch,{ticker},{ticker}/backfill} |
...
| `routers/market_indicators.py` | `/api/market` | 25 | fx·vix·treasury·commodities·econ-indicators·indices·kospi-futures·fear-greed·macro-signals·kospi-signal·m7-earnings·kr-top2-earnings·kr-exports·leverage(+coverage/backfill)·lending(+sync)·refresh-* |
...
| `routers/guru.py` | `/api/guru` | 6 | managers·stats/{popularity,manager-top3,weighted}·crawl(+progress) |
...
| `routers/admin.py` | `/api/admin` | 13 | users(+`{id}/permissions`·bulk-permissions·DELETE)·default-permissions·`DELETE stocks/{ticker}`·analytics/{summary,events,users,users/{id}}·**`PUT analyst-targets/{ticker}`**·**`POST cowork/fire`** |
...
(그래서 발행물은 `/api/analyst-reports` — ADR-0027).
...
| `pages/Guru.jsx` | 매니저 목록 / 인기순 / 매니저별 탑3 / 가중치 | `pages/GuruManagers.jsx` · `pages/GuruStats.jsx`(`view=`) |
...
├── pages/            (33) 라우트 페이지 + 탭 전용 페이지(SectorTab·MacroTab·ExposureTab·RebalanceTab·
│                          Analytics·Market·ReportManualGen·ConsensusSettings·GuruCrawlNow·
│                          LeverageBackfillSettings·GuruManagers·GuruStats·Showcase·LoginPage…)
├── components/
...
│   ├── market/        13 섹션 + marketUtils.jsx + Market.css
│   ├── reports/       21 파일(DetailTab 690·Sections 516·ConsensusChart·FinancialsChart·
...
├── styles/           tokens.css · pc.css · mobile.css · motion.css
...
- `.forge/codebase/` 7문서(ARCHITECTURE·STRUCTURE·STACK·CONVENTIONS·CONCERNS·INTEGRATIONS·TESTING),
  `.forge/adr/0001~0028`(+`retired/`), `.forge/backlog/`·`executed/`·`done/`·`retro/`·`quick/`.
...
- market_cache 키는 소문자 스네이크(`macro_signals`·`kr_sector_momentum`).
- 로그 마커는 `[Component]` PascalCase, 개념당 1스펠링(`[Scheduler]`·`[AnalystReport]`·`[CoworkTrigger]`·`[Migrate]`·`[Cache]`). `print` 금지(`tests/test_no_print.py`).
- 테스트: `backend/tests/test_<대상>.py`(라우터=`test_<name>_router.py`, 배치=`test_<name>_batch.py`).
...
- 라우트 경로는 kebab-case(`/analyst-reports`·`/market/indicators`·`/admin-analytics`).

...
- 드라이버/풀: `psycopg2-binary` + `psycopg2.pool.ThreadedConnectionPool` (`backend/services/db.py` — `_get_pool()`, `RealDictCursor`, `execute_batch`). 헬퍼 `execute()`/쿼리 함수.
...
- **한국투자증권 KIS REST** (`backend/services/kis/`: client·quote) — **KR+US 읽기전용 백업 시세**(키움/yfinance 실패 시, ADR-0011). 베이스 `https://openapi.koreainvestment.com:9443` (env `KIS_BASE_URL` override, 기본 실전). 토큰: env `KIS_APP_KEY` + `KIS_APP_SECRET` → `POST /oauth2/tokenP`(발급 60s 가드, EGW00133 방어). `request(tr_id, path, params)` GET. KR `FHKST01010100`, US `HHDFS00000300`/`HHDFS76240000`. 키 미설정=안전 기본값(휴면).
...
- **multpl.com CAPE 크롤** — `https://www.multpl.com/shiller-pe` 페이지를 `requests`+`BeautifulSoup(html, "html.parser")`로 파싱(`backend/services/market_indicators/indices.py:_parse_multpl_cape`). S&P500 Shiller CAPE(FRED 부재 대체). 키 불필요, 실패 graceful.
...
- **JWT (HS256)** — `backend/services/auth_service.py` (`jwt.encode(..., algorithm="HS256")`, `jwt.decode(..., algorithms=["HS256"])`). env `JWT_SECRET`. 리프레시 토큰은 `refresh_tokens` 테이블.
- **세션 서명** — starlette `SessionMiddleware`, env `SESSION_SECRET` (`itsdangerous`).
- **OAuth** — `backend/routers/auth.py` (`authlib`). Google(`GET /api/auth/oauth/google`, env `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`) + GitHub(env `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`). OAuth 토큰 교환은 임시 code(120s TTL, `_oauth_codes` 인메모리) → 프론트가 code로 교환. (Google at_hash는 base64 직접 디코딩 — jose 검증 우회.)
...
- 인증: API 키 게이팅 — env `COWORK_API_KEY`, 의존성 `require_admin_or_api_key`/`get_current_user_or_api_key` (`backend/routers/report.py`, `_API_KEY_USER_ID`로 매핑). enrich API가 AI 분석 텍스트·insights·`tickers.enriched_at`를 씀. 백엔드 자체엔 LLM 호출 없음(`ANTHROPIC_API_KEY`는 `.env.docker`에 있으나 현재 미사용).
...
- CORS origins (`backend/main.py`): `localhost:3000`, `localhost:5173`, env `FRONTEND_URL`. 배포 시 `FRONTEND_URL`을 `.env.docker`에 설정.

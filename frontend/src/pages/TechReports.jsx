...
// 선도기술 리포트 목록 (ADR-0033, task#276 S5) — 기술 단위 발행물, 기술당 최신 1건.
// 대상 4종 고정(백엔드 TECH_TOPICS 정본)이라 페이지네이션 없음. /analyst-reports 목록과 동형 구조.
...
export default function TechReports() {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <h3 style={{ fontFamily: 'var(--font-serif)', color: 'var(--text)', fontSize: 16, margin: '0 0 10px' }}>선도기술 리포트</h3>
      {reports === null && !error ? (
        <Skeleton variant="card" count={4} />
      ) : error ? (
        <p style={{ color: 'var(--text-3)', fontSize: 13 }}>{error}</p>
      ) : reports.length === 0 ? (
        <p style={{ color: 'var(--text-3)', fontSize: 13 }}>발행된 선도기술 리포트가 없습니다.</p>
      ) : (
          ...
          {reports.map(r => {
            ...
            return (
              ...
              >
                ...
                <div className="mono" style={{ color: 'var(--text-3)', fontSize: 11 }}>{r.published_date} 발행</div>
              </Card>
            )
          })}
        </div>

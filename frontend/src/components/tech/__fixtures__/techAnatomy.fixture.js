// 라이브 실발행분(solid-state-battery)에서 **복사**한 픽스처 — 손으로 지어내지 않는다.
// 지어낸 픽스처는 fixture-pass-live-fail의 입구다(이 저장소가 반복해 밟은 경로).
//
// ⚠️ **2026-08-19 스냅샷이지 parity가 아니다.** 라이브는 이후 늘어난다 — 08-20 프로브 시점의
// 라이브 producers는 이미 이 픽스처(9곳)보다 많았다(task#314가 채우는 중). 그러니 이 파일을
// 「라이브와 같다」로 읽지 말 것: 라이브 집합의 **부분집합**이고, 여기서 통과한다는 사실이
// 라이브를 검증한 것이 아니다(라이브 판정은 프로브가 실행 시점에 계산해야 한다).

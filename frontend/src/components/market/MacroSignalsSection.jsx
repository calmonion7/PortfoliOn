...
export default function MacroSignalsSection() {
  ...
  const series = [
    { key: 'yield_curve', label: '장단기 금리차 (10Y-2Y)', color: '#90caf9', valFmt: v => `${v.toFixed(2)}%p`, chgUnit: '%p', zeroLine: true },
    { key: 'hy_spread', label: '하이일드 스프레드', color: '#ffb74d', valFmt: v => `${v.toFixed(2)}%`, chgUnit: '%p', refLine: 5.0 },
    { key: 'm2', label: 'M2 통화량 (십억$)', color: '#a5d6a7', valFmt: v => v.toLocaleString(undefined, { maximumFractionDigits: 0 }), chgUnit: '%', chgPct: true },
    { key: 'fed_funds', label: '연방기금금리', color: '#ce93d8', valFmt: v => `${v.toFixed(2)}%`, chgUnit: '%p' },
  ]
    ...
    <SectionCard title="매크로 신호 (미국)" summary={summary} open={open} onToggle={() => setOpen(o => !o)}>
      ...
      {(inverted || creditStress) && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {inverted && (
            <div style={{ ...CARD_STYLE, fontSize: 12, color: '#e57373', border: '1px solid #e57373', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>⚠</span> 수익률곡선 역전 (장단기 금리차 &lt; 0)
            </div>
          )}
          {creditStress && (
            <div style={{ ...CARD_STYLE, fontSize: 12, color: '#ffb74d', border: '1px solid #ffb74d', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        {cards.map(({ key, label, color, valFmt, chg, chgUnit, last, prev, zeroLine }) => {
          ...
          return (
            <div key={key} style={{ ...CARD_STYLE, flex: 1, minWidth: 150, ...(warn ? { border: '1px solid #e57373' } : {}) }}>
              <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 4 }}>{label}</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: warn ? '#e57373' : color }}>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={hist} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: 'var(--bg-elev)', border: '1px solid var(--border)', fontSize: 11 }}
                         ...
                         formatter={v => [v, label]} />
                {zeroLine && <ReferenceLine y={0} stroke="#e57373" strokeDasharray="4 4" />}
                {refLine != null && <ReferenceLine y={refLine} stroke="#ffb74d" strokeDasharray="4 4" />}

...
//
// eco: match는 to와 다를 때만 단다. `/analyst-report`(단수) 하나가 목록 `/analyst-reports`까지
// 함께 덮는 것은 **의도**다 — 상세에서 부모 탭을 강조하는 게 이 앱의 관례이고(`/guru/:id`와 동일),
// 접두사 매칭은 앱 전역 관례다. 천장: 형제 항목끼리 접두사 관계가 생기면 그때 세그먼트 경계
// 매칭으로 올려야 한다(현재 5섹션 어디에도 그런 쌍은 없다).
...
export const matchesItem = (pathname, item) => pathname.startsWith(item.match ?? item.to)

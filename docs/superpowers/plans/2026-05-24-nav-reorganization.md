# Nav & Code Reorganization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 8개 상단 내비게이션을 5개로 줄이고, Market.jsx (715줄)를 섹션 컴포넌트로 분리한다.

**Architecture:** `components/market/`에 8개 섹션 파일 + `marketUtils.js` 생성 후 `Market.jsx`를 얇은 셸로 교체. `Research.jsx`와 `MarketHub.jsx` 허브 페이지 신규 생성. `App.jsx` 라우트·내비 5개로 재구성.

**Tech Stack:** React 18, React Router v6, Recharts, Axios, Vite
...
|---|---|---|
| Create | `frontend/src/components/market/marketUtils.js` | 공유 상수(`CARD_STYLE` 등) + 헬퍼(`krFmt`, `isEstimated`) + `LoadingBox`, `ErrorBox` 컴포넌트 |
| Create | `frontend/src/components/market/FxSection.jsx` | 환율 섹션 |
| Create | `frontend/src/components/market/VixSection.jsx` | VIX 섹션 |
| Create | `frontend/src/components/market/CommoditiesSection.jsx` | 원자재 섹션 |
| Create | `frontend/src/components/market/EconIndicatorsSection.jsx` | 경제지표 섹션 |
| Create | `frontend/src/components/market/TreasurySection.jsx` | 미국채 금리 섹션 |
| Create | `frontend/src/components/market/M7EarningsSection.jsx` | M7 순이익 섹션 |
| Create | `frontend/src/components/market/KrTop2Section.jsx` | KOSPI Top2 섹션 |
| Create | `frontend/src/components/market/KrExportsSection.jsx` | 한국 수출 섹션 |
| Rewrite | `frontend/src/pages/Market.jsx` | 섹션 조합 셸 (~20줄) |
| Create | `frontend/src/pages/Research.jsx` | 리서치 허브 (탭: 리포트·캘린더·다이제스트) |
| Create | `frontend/src/pages/MarketHub.jsx` | 시장 허브 (탭: 시장지표·분석) |
| Modify | `frontend/src/App.jsx` | 라우트 5개로 재구성 |

---

## Task 1: marketUtils.js 생성

**Files:**
- Create: `frontend/src/components/market/marketUtils.js`
...
`frontend/src/components/market/marketUtils.js` 내용:
...
```bash
git add frontend/src/components/market/marketUtils.js
git commit -m "refactor: market 공유 상수·헬퍼 marketUtils.js로 추출"
```

---

## Task 2: FxSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/FxSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/FxSection.jsx` 내용:
...
```bash
git add frontend/src/components/market/FxSection.jsx
git commit -m "refactor: FxSection 컴포넌트 분리"
```

---

## Task 3: VixSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/VixSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/VixSection.jsx` 내용:
...
```bash
git add frontend/src/components/market/VixSection.jsx
git commit -m "refactor: VixSection 컴포넌트 분리"
```

---

## Task 4: CommoditiesSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/CommoditiesSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/CommoditiesSection.jsx` 내용:
...
```bash
git add frontend/src/components/market/CommoditiesSection.jsx
git commit -m "refactor: CommoditiesSection 컴포넌트 분리"
```

---

## Task 5: EconIndicatorsSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/EconIndicatorsSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/EconIndicatorsSection.jsx` 내용:
...
```bash
git add frontend/src/components/market/EconIndicatorsSection.jsx
git commit -m "refactor: EconIndicatorsSection 컴포넌트 분리"
```

---

## Task 6: TreasurySection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/TreasurySection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/TreasurySection.jsx` 내용:
...
```bash
git add frontend/src/components/market/TreasurySection.jsx
git commit -m "refactor: TreasurySection 컴포넌트 분리"
```

---

## Task 7: M7EarningsSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/M7EarningsSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/M7EarningsSection.jsx` 내용:

```jsx
import { useState, useEffect } from 'react'
import axios from 'axios'
import { LineChart, Line, LabelList, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts'
import { CARD_STYLE, SECTION_STYLE, SECTION_HEADER_STYLE, DESC_STYLE, LoadingBox, ErrorBox, isEstimated } from './marketUtils'
...
```bash
git add frontend/src/components/market/M7EarningsSection.jsx
git commit -m "refactor: M7EarningsSection 컴포넌트 분리"
```

---

## Task 8: KrTop2Section.jsx 추출

**Files:**
- Create: `frontend/src/components/market/KrTop2Section.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/KrTop2Section.jsx` 내용:

```jsx
import { useState, useEffect } from 'react'
import axios from 'axios'
import { LineChart, Line, LabelList, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts'
import { CARD_STYLE, SECTION_STYLE, SECTION_HEADER_STYLE, DESC_STYLE, LoadingBox, ErrorBox, krFmt, isEstimated } from './marketUtils'
...
```bash
git add frontend/src/components/market/KrTop2Section.jsx
git commit -m "refactor: KrTop2Section 컴포넌트 분리"
```

---

## Task 9: KrExportsSection.jsx 추출

**Files:**
- Create: `frontend/src/components/market/KrExportsSection.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/components/market/KrExportsSection.jsx` 내용:
...
```bash
git add frontend/src/components/market/KrExportsSection.jsx
git commit -m "refactor: KrExportsSection 컴포넌트 분리"
```

---

## Task 10: Market.jsx 셸로 교체

**Files:**
- Rewrite: `frontend/src/pages/Market.jsx`

- [ ] **Step 1: Market.jsx 전체 교체**

`frontend/src/pages/Market.jsx` 내용:
...
- [ ] **Step 2: 브라우저에서 `/market` 접속해 모든 섹션이 정상 렌더링되는지 확인**

- [ ] **Step 3: 커밋**

```bash
git add frontend/src/pages/Market.jsx
git commit -m "refactor: Market.jsx를 섹션 컴포넌트 조합 셸로 교체"
```

---

## Task 11: Research.jsx 생성
...
`frontend/src/pages/Research.jsx` 내용:
...
git commit -m "feat: Research 허브 페이지 생성 (리포트·캘린더·다이제스트 탭)"
```

---

## Task 12: MarketHub.jsx 생성

**Files:**
- Create: `frontend/src/pages/MarketHub.jsx`

- [ ] **Step 1: 파일 생성**

`frontend/src/pages/MarketHub.jsx` 내용:
...
```bash
git add frontend/src/pages/MarketHub.jsx
git commit -m "feat: MarketHub 허브 페이지 생성 (시장지표·분석 탭)"
```

---

## Task 13: App.jsx 업데이트
...
- [ ] **Step 2: 브라우저에서 5개 내비 항목 전체 동작 확인**
  - `/` → 종목관리 정상 렌더
  - `/research` → 리포트·캘린더·다이제스트 탭 전환 동작
  - `/market` → 시장지표·분석 탭 전환 동작, 시장지표 내 8개 섹션 모두 렌더
  - `/guru` → 매니저·통계 탭 정상 동작
  - `/settings` → 리포트 설정·구루 설정 탭 정상 동작

- [ ] **Step 3: 커밋**

```bash
git add frontend/src/App.jsx
git commit -m "feat: 내비게이션 8개 → 5개로 재구성 (리서치·시장 허브 통합)"
```

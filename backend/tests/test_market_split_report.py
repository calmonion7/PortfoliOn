...
def test_kr_after_2030_returns_today(monkeypatch):
    ...
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_2030)
    monkeypatch.setattr(storage, "get_batch_schedule", lambda jid: _sched("20:30"))
...
def test_kr_before_2030_returns_prev_weekday(monkeypatch):
    ...
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_1000)
    monkeypatch.setattr(storage, "get_batch_schedule", lambda jid: _sched("20:30"))
...
def test_us_after_0700_returns_today(monkeypatch):
    ...
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_0800)
    monkeypatch.setattr(storage, "get_batch_schedule", lambda jid: _sched("07:00"))
...
def test_us_before_0700_returns_prev_weekday(monkeypatch):
    ...
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_0600)
    monkeypatch.setattr(storage, "get_batch_schedule", lambda jid: _sched("07:00"))
...
def test_disabled_schedule_returns_today(monkeypatch):
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_1000)
    monkeypatch.setattr(storage, "get_batch_schedule", lambda jid: _sched("20:30", enabled=False))
    assert storage.expected_report_date("KR") == "2026-06-15"
...
def test_expected_report_dates_returns_both_markets(monkeypatch):
    ...
    monkeypatch.setattr(storage, "_now_kst", lambda: _MON_1000)  # 10:00: US 07:00은 지남, KR 20:30은 전

    def sched(jid):
        return _sched("20:30") if jid == "daily_report_kr" else _sched("07:00")

    monkeypatch.setattr(storage, "get_batch_schedule", sched)

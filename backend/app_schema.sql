...
-- 대상은 백엔드 상수 TECH_TOPICS 15종이 정본(ADR-0039 1차 개정 · ADR-0044 2차 개정) — 목록을
-- 여기 복제하지 않는다(복제하면 다음 개정마다 드리프트한다). 편입 판정은 「지금 투자 지형에서
-- 중요한가」이고, 넓은 이름은 한 문서가 감당할 범위로 좁혀 등재한다(ADR-0044 결정 2·3).
-- slug당 1행(ADR-0038) — 갱신은 그 행을 upsert한다(ON CONFLICT (slug) DO UPDATE). published_date는 마지막 갱신일.
-- 단 그 「덮어쓰기의 입도」는 전 컬럼이 아니다(ADR-0038 보정, task#313): 선택 5필드는 요청이 키를
-- 생략하면 SET 목록에서 빠져 직전 판이 보존되고, 지우려면 명시적 null을 보낸다. 그 5필드의 정본
-- 목록은 services/tech_reports.py::_PRESERVABLE이다 — 여기 복제하지 않는다.

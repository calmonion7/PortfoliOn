...
- **KR 리포트의 시세 소스(`get_quote_kr`)와 일봉 차트 소스(`get_history_df`)는 다른 TR이라 스케일이 어긋날 수 있다 — 매물대/RSI가 "깨져" 보이면 표시 버그가 아니라 박제된 price 값 자체를 의심.** 리포트 상세의 현재가 마커는 `get_quote_kr`(키움 ka10001 `_AL`→KIS→Naver), 매물대/RSI는 `get_history_df`(키움 ka10081 일봉)에서 온다. 한쪽만 액면/병합 조정되면 둘이 최대 5배까지 어긋나, 같은 차트에 마커가 매물대 밴드 밖으로 찍혀 깨진 듯 보인다(005930이 ~70k=실값 354k의 1/5로 박제된 사례). 방어: `get_quote_kr`에 **발산 가드**(`_kr_pick_basic`/`_price_sane`, `services/market/kr.py`) — 고른 현재가가 ① **전일종가(prev_close)의 ±30%**(KR 일일 가격제한폭), ② 키움 일봉 종가의 `[ref/2, ref*2]`(독립 TR 교차검증), 또는 ③ **독립 KRX 평문코드 시세**(`_kr_basic_kiwoom(regular=True)`, ±2x, task#96)의 범위 밖이면 그 소스를 폐기하고 다음 소스로 폴백. 셋 다 통과해야 정상 — ①이 정밀(글리치 시에도 base_pric/prev_close는 안정적이라 좋은 앵커), ②는 소스 prev_close까지 오염된 경우 완충, ③은 NXT `_AL`이 quote·prev_close·일봉ref를 *모두* 같은 오스케일로 자기일관 오염시켜 ①②를 둘 다 통과하는 **전체오염**을 잡는다(KRX는 다른 거래소 피드라 동시 동일오염 사실상 없음). ③은 `get_quote_kr(regular=False)`에만(리포트 regular=True는 이미 KRX라 스킵·비용 무변), 모든 라이브 소스 폐기 시 글리치 NXT 대신 KRX 참조를 반환. **잔존(KRX-poison)**: ③이 KRX를 앵커로 신뢰하므로 *KRX*가 글리치나면 정상 NXT를 false-reject한다 — task#94 근거(KRX 안정)로 net-positive 채택, 단일 참조 글리치 면역화는 2-of-3 다수결 후속 후보. 대시보드 핫패스(`get_quotes_batch`/`_changes_from_closes`)는 이 가드를 안 타며(ephemeral 용인) task#96 무변경. 소수 괴리 소스만 폐기(전원 합의 시 미발동), 참조 부재 시 해당 검증만 생략. 진단 시 네이버 공개 API(`m.stock.naver.com/api/stock/{code}/basic`)로 creds 없이 실값 대조 + 컨테이너에서 KRX(`005930`) vs SOR(`005930_AL`) 원값 직접 비교(`docker exec -i portfolion-backend-1 python - < probe.py`). **근본원인(task#94)**: 005930 ~70k는 영속 버그가 아니라 **NXT `_AL`(SOR 통합코드) 순간 이상체결**이 일배치에 박제된 *일시적* 값 — 영속 소스 버그로 단정해 소스부터 고치려 들지 말 것. 부수: 키움 `_AL`은 평시에도 NXT 시간외가(KRX 정규장과 ~1% 차)를 반환. (task#93·94)
- **시세 기준 이원화 — 리포트 스냅샷=KRX 정규장 / 라이브 대시보드=NXT(task#95, ADR-0020)**: 키움 코드선택 단일 분기점 `client.integrated_code(stk_cd, regular=False)`에 `regular` 플래그가 있다 — 기본 False=`_AL`(NXT 시간외), `regular=True`=평문 KRX 코드(정규장 종가). 시세 체인(`get_quote`→…→`get_basic_info`)·차트 체인(`get_history_df`→…→`fetch_bars`)으로 전파되고, **리포트 스냅샷 writer만 `regular=True`로 opt-in**(`report_generator` 당일/백필 daily_df·get_quote·경쟁사, `report.py:refresh_analyst`). RSI(weekly/monthly_df·`indicators`)·대시보드(`get_quotes_batch`)·종목추가·`resolve_name`은 NXT 기본 유지. `get_quote` TTL 캐시 키에 `regular` 포함(정규장/NXT quote 충돌 방지). → 같은 종목이 리포트(354k)와 대시보드(350.5k)에 ~1% 다른 현재가를 보일 수 있는 건 의도된 기준 차. **이게 리포트 박제 70k의 근본해결**(리포트가 `_AL` 글리치에 노출 안 됨) — 위 발산 가드는 이제 대시보드(NXT) 백스톱. **단, 가드는 자기일관적 `_AL` 전체오염(quote·prev_close·일봉ref가 다 같은 오스케일)은 못 잡는다**(±30%·±2x 둘 다 통과). 또 **"fix 배포는 이미 박제된 스냅샷을 소급 치료하지 않는다"** — 배포 전 일배치가 박제한 stale 70k는 *재생성*해야 KRX로 덮인다(재생성 전 프로브로 라이브 소스 깨끗 확인 → transient 글리치 재박제 방지).

...
- **배치 id를 `batch_registry.BATCHES`에서 빼면 그 id를 쓰는 *모든* 표면을 전수 grep**: ① 데이터 read(스케줄 소비처), ② 표시 문자열(`schedule_desc` 등), ③ **`job_runs.record(id, ...)` 모든 lane — auto뿐 아니라 manual·backfill 경로까지**, ④ 그 id를 단언하는 테스트. 한 곳이라도 옛 id로 남으면 stale read(과거)·**배치 현황 실행이력에서 사라지는 회귀**(이번: 수동생성/백필이 은퇴한 `daily_report`로 record → 카드에서 증발, daily_report-market-split task15·17·45 재발)·고아 run 누적이 생긴다. 특히 옛 id를 단언하던 테스트는 깨진 동작을 고정해 TDD green이 회귀를 못 잡으니(이번엔 적대적 리뷰가 포착), id 은퇴 시 테스트도 grep 대상.

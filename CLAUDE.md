...
- **리포트 당일 경로에서 같은 일봉을 두 경로로 각각 fetch하면 부분 결측이 조용히 난다(task#126)** — `report_generator.generate_report`는 daily 계열 지표(EMA·52주·HV·매물대)를 직렬로 먼저 받은 `daily_df`(US=`_t.history(period="1y")`)로 계산하지만, **RSI만 `indicators.get_timeframe_rsi`가 내부에서 `market.get_history_df(...)`로 *같은 일봉을 별도·ThreadPool 동시* 재fetch**했다(파라미터는 `_HISTORY_CFG["daily"]`=1y/1d로 동일 → 순수 중복). 8개 동시 호출 중 이 fetch만 rate-limit되면 **RSI만 null**·나머지 지표는 멀쩡 → "요약 탭 '매물대 & RSI 현황' 차트는 뜨는데 지표>기술·수급의 'RSI 예상 타점'만 빈 헤더 박스"(500도 안 나고 조용히 부분 결측 — 라이브 전용, fixture 미포착, #116 tz-strip과 같은 fixture-pass-live-fail 가족). 수정: `get_timeframe_rsi(ticker, market, exchange, daily_df=None)`로 daily는 넘겨받은 daily_df 재사용(US만 전달; KR daily RSI는 NXT 유지가 기존 non-goal). **잔존 위험**: weekly/monthly RSI는 5y/10y가 필요해 여전히 별도 fetch → 동일 부분결측 가능(프론트 `RsiTable`의 per-timeframe `available` 필터가 흡수). 교훈: **이미 받은 df가 있으면 재fetch 말고 재사용**(백필 경로 `backfill_ticker`의 `_rsi_block(d_trim)`는 이미 그렇게 함 — 당일 경로만 어긋나 있었음). 프론트 방어: `RsiTable`은 세 타임프레임 rsi가 전무하면 빈 헤더 박스 대신 미렌더(`available.length===0 → return null`).

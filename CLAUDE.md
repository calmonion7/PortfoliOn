...
- `backend/services/backlog.py` — 수주잔고(Order Backlog) 수집. DART엔 수주잔고 전용 구조화 API가 없어 **공시서류원본파일 `/api/document.xml`**(ZIP→전 멤버 디코드)로 원문을 받아 "수주" 블록을 `source='pending'`+`raw_text`로 적재하고, 수치는 Claude Cowork(`PUT /api/report/{ticker}/backlog`)가 채운다(순수 pending, 자동 금액 파싱 없음). KR 전용·`DART_API_KEY` 필수. 전 종목 배치는 `backlog_fetch`(주간 일 04:00) / 수동 `POST /api/report/backlog/refresh-all`(admin). 근거: `.forge/adr/0002`.

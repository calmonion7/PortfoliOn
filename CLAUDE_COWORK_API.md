...
> | 산업 | 핵심 자원 | 대표 지표 |
> |------|-----------|-----------|
> | 제약·바이오 | R&D 파이프라인 | 파이프라인 단계별 개수, R&D 집약도 |
> | 반도체·하드웨어 | 생산능력 | 가동률, CapEx |
> | 소프트웨어·플랫폼 (IT서비스·컨설팅 포함) | 인력 | 직원수, 1인당 매출, 1인당 영업이익 (완전 명세는 아래) |
...
> | 금융 | 자기자본 | NIM(순이자마진), ROE — drivers: 자본비율(CET1 등), 배당여력 |
> | 소비재·유통 | 브랜드·매장망 | 동일매장매출 성장률(SSSG), 재고회전율 |
> | 미디어·게임 | IP | 주요 IP 매출 비중, 신규 IP 파이프라인 |
> | 통신·유틸리티 | 네트워크/설비 자산 | ARPU, 설비투자(CapEx) 효율 |
>
